...
import NavigatorView from './pattern-navigator-view';
...
export default {
  ...
  config: {
    match: {
      ...
      description: 'RegExp to match lines shown in navigator',
      type: 'string',
      default: '^\\s*\\S+\\s*(:=|=|:)|\\bclass\\b|\\bfunction\\b|[-~=]>|{\\s*$|\\bTODO\\b|\\?\\?\\?|!!!|-{5,}|={5,}|\\*{5,}'
    },
    autofold: {
    ...
    maxLineContentLength: {
      ...
      description: 'Navigator will take the line on which the fold is on but if the line is longer than this many characters then it will truncate it. config.cson key : maxLineContentLength',
      type: 'integer',
      default: 60
    },
    minLineLength: {
    ...
    autoScrollNavigatorPanel: {
      title: 'Auto scroll fold navigator panel',
      description: 'Scrolls the fold navigator panel to the active fold. config.cson key : autoScrollNavigatorPanel',
      type: 'boolean',
      default: true
    },
    unfoldAllSubfolds: {
    ...
    maxFoldLevel: {
      title: 'Maximum fold level navigator will show',
      description: 'With this option you can limit the fold level depth we list on the panel for a better overview of the code. config.cson key : maxFoldLevel',
      type: 'integer',
      default: 2,
    },
    log: {
  ...
  activate(state) {
    ...
    */
    ...
    this.navigatorView = new NavigatorView(state.navigatorViewState);
  ...
  alignEditorToSearch() {
    var selectedArr = this.searchModalItems.getElementsByClassName('pattern-navigator-search-modal-item-selected');
    var selected = selectedArr[0];
    var editor = atom.workspace.getActiveTextEditor();
    var row = selected ? parseInt(selected.dataset.row) : -1;
    if (row >= 0) {
  ...
  searchModalOnClick(event) {
    ...
    if (event.target.matches('.pattern-navigator-search-modal-item')) {
      clicked = event.target;
    } else if (event.target.matches('.pattern-navigator-indentation') && event.target.parentNode && event.target.parentNode.matches('.pattern-navigator-search-modal-item')) {
    ...
    var row = parseInt(clicked.dataset.row);
  ...
  addSearchModal() {
    ...
    this.searchModalElement.classList.add('pattern-navigator-search-modal');
    ...
    this.searchModalInput.addEventListener('input', () => {
      ...
      filteredItems.forEach((item, index) => {
        ...
        let selected = ' pattern-navigator-search-modal-item-selected';
        if (index > 0) {
          selected = '';
        }
        ...
        for (let j = 0; j < item.indentation; j++) {
          indentHtml += '<span class="pattern-navigator-indentation">' + this.settings.indentationCharacter + '</span>';
        }

        html += '<div class="pattern-navigator-search-modal-item pattern-navigator-item-indent-' + item.indentation + selected + '" data-row="' + item.line + '">' + indentHtml + item.content + '</div>';
      });
    ...
    this.searchModalElement.addEventListener('keydown', (e) => {
      ...
      var sc = 'pattern-navigator-search-modal-item-selected';

      if (e.keyCode === 38 || e.keyCode === 40 || e.keyCode == 13 || e.keyCode == 27) {
        var items = this.searchModalItems.getElementsByClassName('pattern-navigator-search-modal-item');
        ...
        if (e.keyCode === 38) {
          ...
          // enter
          if (selected) {
            var row = parseInt(selected.dataset.row);
            if (row >= 0) {
  ...
  onDidChangeCursorPosition(event) {
    //console.log(arguments.callee.name);
    this.setCursor(event.newBufferPosition);
  },

  addNavigatorPanel() {
    //console.log(arguments.callee.name);
    var element = this.navigatorView.getElement();
  ...
  iniSettings() {
    ...
    if (editor) {
      ...
      languageSettings = atom.config.get('pattern-navigator', {
        'scope': [scope]
      });
    }

    this.settings = atom.config.get('pattern-navigator');
  ...
  registerCommands() {
    //"ctrl-alt-cmd-up": "pattern-navigator:previousFoldAtCurrentLevel",
    //"ctrl-alt-cmd-down": "pattern-navigator:nextFoldAtCurrentLevel",
    //"ctrl-alt-cmd-up": "pattern-navigator:previousFold",
    //"ctrl-alt-cmd-down": "pattern-navigator:nextFold",
    //
    //console.log(arguments.callee.name);
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggle': () => this.toggle()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:open': () => this.open()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:close': () => this.close()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:previousFold': () => this.previousFold()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:nextFold': () => this.nextFold()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:moveLevelUp': () => this.moveLevelUp()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:moveLevelDown': () => this.moveLevelDown()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:nextFoldAtCurrentLevel': () => this.nextFoldAtCurrentLevel()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:previousFoldAtCurrentLevel': () => this.previousFoldAtCurrentLevel()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:unfoldSubfolds': () => this.unfoldSubfoldsPublic()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:foldActive': () => this.foldActivePublic()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:unfoldAtLevel1': () => this.unfoldAtLevel(1)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:unfoldAtLevel2': () => this.unfoldAtLevel(2)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:unfoldAtLevel3': () => this.unfoldAtLevel(3)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:unfoldAtLevel4': () => this.unfoldAtLevel(4)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:unfoldAtLevel5': () => this.unfoldAtLevel(5)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:foldAtLevel1': () => this.foldAtLevel(1)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:foldAtLevel2': () => this.foldAtLevel(2)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:foldAtLevel3': () => this.foldAtLevel(3)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:foldAtLevel4': () => this.foldAtLevel(4)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:foldAtLevel5': () => this.foldAtLevel(5)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:maxFoldLevel1': () => this.maxFoldLevel(1)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:maxFoldLevel2': () => this.maxFoldLevel(2)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:maxFoldLevel3': () => this.maxFoldLevel(3)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:maxFoldLevel4': () => this.maxFoldLevel(4)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:maxFoldLevel5': () => this.maxFoldLevel(5)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggleFoldsLevel1': () => this.toggleFoldsLevel(1)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggleFoldsLevel2': () => this.toggleFoldsLevel(2)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggleFoldsLevel3': () => this.toggleFoldsLevel(3)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggleFoldsLevel4': () => this.toggleFoldsLevel(4)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggleFoldsLevel5': () => this.toggleFoldsLevel(5)
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggleSearch': () => this.toggleSearch()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'pattern-navigator:toggleActiveFold': () => this.toggleActiveFold()
    }));
  ...
  previousFold() {
    ...
    //console.log(arguments.callee.name);
    this.clearHistory();
    var fold = this.navigatorView.getActiveFold();
  ...
  parse(editor) {
    ...
    this.navigatorView.clearContent();
    this.clearHistory();
    this.lines2fold = [];
    this.folds = [];
    this.visibleFolds = [];
    ...
    for (var i = 0; numberOfRows > i; i++) {
      ...
      let lineContent = editor.lineTextForBufferRow(i);

      var matching = lineContent.match(this.settings.match);
      var foldable = editor.isFoldableAtBufferRow(i);

      if (matching || foldable) {
        let parent;
      ...
      if (matching) {
        ...
        let lineNumberSpan = "";
        //let lineContent = "";
        let classList = "pattern-navigator-item";
        let id = "";
        let gutter = '<span class="pattern-navigator-gutter"></span>';
        let content = '';
        ...
        if (this.settings.showLineNumbers) {
          lineNumberSpan = '<span class="pattern-navigator-line-number ">' + (i + 1) + '</span>';
        }

        id = 'pattern-navigator-item-' + i;
        classList += ' pattern-navigator-item-' + i;
        classList += ' pattern-navigator-item-indent-' + indentLevel;
    ...
    this.navigatorView.setContent(html);
    this.setCursor(editor.getCursorBufferPosition());
  },
  ...
  toggle() {
    return (this.panel.isVisible() ? this.close() : this.open());
  },
  ...
  setCursor(point) {
    ...
    var editor = atom.workspace.getActiveTextEditor();
    if (editor) {
      if (this.settings.keepFoldingAllTime)
        editor.foldAll();
      this.unfoldSubfolds(point.row);
    }

    var fold = this.lines2fold[point.row];
    var line = this.navigatorView.selectFold(fold);

    // autoscroll navigator panel
    if ((line) && !this.wasItOnClick && this.settings.autoScrollNavigatorPanel) {
  ...
  navigatorElementOnClick() {
    ...
    element.onclick = (event) => {
      ...
      var clicked = null;
      if (event.target.matches('.pattern-navigator-item'))
        clicked = event.target;
      else if (event.target.matches('.pattern-navigator-indentation') && event.target.parentNode && event.target.parentNode.matches('.pattern-navigator-item'))
        clicked = event.target.parentNode;
  ...
  unfoldSubfolds(row = false, editor = false, force = false, level = 0) {
    //console.log(arguments.callee.name);

    var editor = editor ? editor : atom.workspace.getActiveTextEditor();
    if (!editor)
      return;
    var fold = (row || row === 0) ? row : this.navigatorView.getActiveFold();
    ...
    if (foldObj.children.length > 0) {
      foldObj.children.forEach(
        (value) => {
          this.unfoldSubfolds(value, editor, true, level+1)
        }
      );
    }
  },
  ...
  foldAtLevel(level) {
    ...
    editor.foldAllAtIndentLevel(level-1);
    this.parse(editor);
  },
  ...
  maxFoldLevel(level) {
    ...
    atom.config.set("pattern-navigator.maxFoldLevel", level);
    this.parse(editor);
  },
